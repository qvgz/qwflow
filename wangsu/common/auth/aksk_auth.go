...
import (
	...
	"fmt"
	"net/url"
	"qwflow/wangsu/common/constant"
	"qwflow/wangsu/common/model"
	"qwflow/wangsu/common/util"
	"sort"
	"strconv"
	"strings"
	"time"
...
func Invoke(config AkskConfig, jsonStr string) []byte {
...
func getQueryString(requestMsg model.HttpRequestMsg) string {
	indexOfQueryStringSeparator := strings.Index(requestMsg.Uri, "?")
	if "POST" == requestMsg.Method || -1 == indexOfQueryStringSeparator {
		return ""
	}
